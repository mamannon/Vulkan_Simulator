...
static void JsonParse(JsonDocument& doc, const char* str, size_t length,
    bool throwExc = false);
...
static std::string GetKey(json_const_iterator& it);
...
static std::string JsonToString(const json& o, int spacing = -1);
static void swap4(unsigned int* val);
static std::string JoinPath(const std::string& path0,
    const std::string& path1);
static std::string FindFile(const std::vector<std::string>& paths,
    const std::string& filepath, tinygltf::FsCallbacks* fs);
...
static std::string JsonToString(const json& o, int spacing) {
...
static inline std::wstring UTF8ToWchar(const std::string& str) {
    ...
    return cvt.from_bytes(str.c_str());
}
...
namespace tinygltf {
    ...
    static bool ParseNode(Node* node, std::string* err, const json& o,
        bool store_original_json_for_extras_and_extensions);
    bool Equals(const std::vector<double>& one,
        const std::vector<double>& other);
    bool Equals(const Value& one, const Value& other);
    ...
    std::string ExpandFilePath(const std::string& filepath, void*) {
        ...
        std::replace(wfilepath.begin(), wfilepath.end(), '/', '\\');

        return WcharToUTF8(wfilepath);
    ...
    bool ReadWholeFile(std::vector<unsigned char>* out, std::string* err,
        ...
        // `wchar_t *`
        std::wstring temp = UTF8ToWchar(filepath);
        std::ifstream f(temp.c_str(), std::ifstream::binary);
        ...
        if (is_binary) {
            ...
            if (!buffer->uri.empty()) {
                ...
                if (IsDataURI(buffer->uri)) {
                    ...
                    // External .bin file.
                    std::string decoded_uri = urldecode(buffer->uri);
